import pytest
from models import ReadingBase, ReadingType
...
def test_reading_creation():
    ...
    assert reading.name == "Тест"
    assert reading.reading_type == ReadingType.Plain
    assert reading.url == ""

def test_text_fragmentize_throw_exception():
    reading = ReadingBase(
        name="Test",
        reading_type=ReadingType.Plain,
        text = ""
    )
...
def test_test_text_fragmentize():
    reading = ReadingBase(
        name="Test",
        reading_type=ReadingType.Plain,
        text = test_text
    ...
    sentenses = reading.fragmentize()
    assert len(sentenses) == 23